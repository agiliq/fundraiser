...
def SendEmail(sub, msg, to, user=None):
	...
	if not sub=="approve_sub":
		...
		if user.profile.is_beneficiary:
			admin_msg = render_to_string("email/admin_body.html", {"username":user.username, "ben":True})
		else:
			admin_msg = render_to_string("email/admin_body.html", {"username":user.username, "donr":True})
