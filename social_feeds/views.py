...
def invite_gmail_contacts(request):
    ...
    if 'q' in request.GET and request.GET['q']:
        ...
        filtered_contacts = [contact.decode('utf-8')
                             ...
                             if search_string in contact.decode('utf-8')]
...
def send_invitation_to_gcontacts(request):
    ...
    if request.method == 'POST':
        if 'invite' in request.POST:
            ...
            massemail.delay('gmail_invite', 'gm_invite_msg',
                            email_list, request.user)
            messages.add_message(request, messages.SUCCESS,
                                 'Your invitations has been\
                                 sent to the selected users !!!!')
            # return HttpResponseRedirect(reverse('index:home'))
        else:
            error = True
    return render_to_response('social_feeds/invitations_sent.html',
                              {'error': error},
                              context_instance=RequestContext(request))
