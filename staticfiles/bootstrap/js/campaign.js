$(document).ready(function() {
    ...
    $(document).on('keydown', 'input[name^=fund-dist-amt]', function (e) {
        ...
        var id_target_amount = $("input#id_target_amount");
        if ((parseInt(id_target_amount.val(), 10) == 0.0) || (id_target_amount.val().trim() == "")) {
            error+=1;
            id_target_amount.attr("class", "required");
            id_target_amount.siblings("span").html("Target Amount can not be zero or blank");
        }
        else {
            id_target_amount.removeAttr("class");
            id_target_amount.siblings("span").html("");
        }       
